"""Script loader and fault-tolerant diagnostic engine for QCanvas Python scripts."""
...
class ScriptLoadError(Exception):
    """Exception raised when loading a QCanvas Python script fails."""
...
def load_design_from_script(filepath: str | Path) -> Design:
    """Execute a Python script and extract its QCanvas Design instance with error diagnostics."""
    ...
    raise ScriptLoadError(
        "No QCanvas Design instance found in script. Define a 'build_design()' function or a global 'design = PlanarDesign()' variable.",
        filepath=path,
