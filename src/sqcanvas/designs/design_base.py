...
from sqcanvas.config import CHIP_DEFAULTS, DEFAULT_EXPORTER, DESIGN_DEFAULTS
from sqcanvas.exporters import Exporter
from sqcanvas.shapes import ShapeRecord, ShapeStore
from sqcanvas.utility import AttrDict

if TYPE_CHECKING:
    from sqcanvas.components import Component
...
class Design:
    ...
    A design holds:
      * the named :class:`~sqcanvas.components.Component` instances,
      * a :class:`~sqcanvas.shapes.ShapeStore` that accumulates their shapes,
      * chip metadata (size, material) and design variables,
      * access to the pluggable :class:`~sqcanvas.exporters.Exporter` registry.
    ...
    *add* to it. This is what keeps shape authoring and exporting decoupled.
    ...
    def open_gui(self) -> Any:
        ...
        from sqcanvas.gui import launch
